    ...
    <>
      ...
      <SectionShell title="مجموعات الشنط الرئيسية">
        <div className="grid gap-4 md:grid-cols-2">
          {[{title:'شنط خيط الهيمالايا',en:'Himalayan Thread Bags',prices:['Small — 300 EGP','Medium — 400 EGP','Large — 470 EGP'],href:'/shop?collection=himalayan-thread-bags',image:'/images/bahja/redesigned/light-grey-himalayan/light-grey-himalayan-thread-bag-card.png'},{title:'شنط خيوط السلسلة',en:'Chain Thread Bags',prices:['Small — 300 EGP','Medium — 400 EGP','Large — 470 EGP'],href:'/shop?collection=chain-thread-bags',image:'/images/bahja/redesigned/black-gold-chain/black-gold-chain-thread-bag-card.png'}].map((c)=><Link key={c.title} href={c.href} className="bahja-card p-3"><div className="relative h-52 rounded-2xl bg-bahja-cream"><ProductImage src={c.image} alt={c.title} categorySlug="handmade-bags" usage="feature" /></div><div className="space-y-1.5 p-2"><h3 className="text-lg font-semibold">{c.title}</h3><p className="text-xs text-bahja-taupe">{c.en}</p><div className="rounded-xl bg-bahja-cream/70 p-2 text-xs leading-6 sm:text-sm">{c.prices.map((s)=><p key={s}>{s}</p>)}</div></div></Link>)}
        </div>
      </SectionShell>
