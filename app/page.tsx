...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <SectionShell title="لوحات كانفس بروح فنية">{canvasPiece && <Link href={`/shop/${canvasPiece.slug}`} className="grid gap-4 overflow-hidden rounded-3xl border border-bahja-beige bg-white/80 p-4 shadow-soft md:grid-cols-2"><div className="relative aspect-[4/3] rounded-2xl bg-bahja-cream"><ProductImage src={canvasPiece.image} alt={canvasPiece.arabicTitle ?? canvasPiece.title} categorySlug="canvas-art" usage="feature" /></div><div className="my-auto"><p className="text-lg font-medium text-bahja-brown">{canvasPiece.arabicTitle}</p><p className="mt-2 text-sm text-bahja-taupe">لوحات كانفس بروح فنية دافئة، تجمع بين الخط العربي والزهور والتفاصيل اليدوية.</p></div></Link>}</SectionShell>
