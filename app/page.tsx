...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <SectionShell title="لوحات كانفس بروح فنية">{canvasPiece && <Link href={`/shop/${canvasPiece.slug}`} className="block overflow-hidden rounded-3xl border border-bahja-beige bg-white/80 shadow-soft"><div className="relative aspect-[16/7] bg-bahja-cream"><Image src={canvasPiece.image} alt={canvasPiece.arabicTitle ?? canvasPiece.title} fill className="object-cover"/></div><div className="p-5"><p className="text-lg font-medium text-bahja-brown">{canvasPiece.arabicTitle}</p><p className="text-sm text-bahja-taupe">لوحات كانفس بروح فنية دافئة، تجمع بين الخط العربي والزهور والتفاصيل اليدوية.</p></div></Link>}</SectionShell>
      <SectionShell title="طلبات خاصة" subtitle="اختاري اللون، المقاس، نوع السلسلة، والتفاصيل التي تناسب ذوقكِ."><WhatsAppButton href={getWhatsAppUrl(whatsappMessages.customOrderInquiry)} className="w-full sm:w-auto">ابدئي طلبًا خاصًا</WhatsAppButton></SectionShell>
      <SectionShell title="هل لديكِ فكرة لقطعة خاصة؟" subtitle="أرسلي لنا التفاصيل عبر واتساب وسنساعدكِ في اختيار الأنسب."><WhatsAppButton href={getWhatsAppUrl(whatsappMessages.generalContact)} className="w-full sm:w-auto">اطلبي عبر واتساب</WhatsAppButton></SectionShell>
    </>
  );
}
