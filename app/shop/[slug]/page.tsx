...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <section className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-2 lg:px-8">
    <div className="relative h-[320px] overflow-hidden rounded-3xl bg-bahja-cream sm:h-[460px]"><ProductImage src={product.image} alt={getProductArabicTitle(product.arabicTitle, product.title)} categorySlug={product.categorySlug} usage="detail" /></div>
    <div className="space-y-4"><p className="text-sm text-bahja-taupe">{product.collection}</p><h1 className="text-3xl font-semibold">{getProductArabicTitle(product.arabicTitle, product.title)}</h1><p className="text-xs text-bahja-taupe">{product.title}</p><p className="text-bahja-taupe">{product.description}</p><p className="rounded-2xl bg-bahja-cream/80 p-3 text-sm">{formatArabicPriceGuide(product.priceGuide)}</p>{isBag && <div className="grid grid-cols-2 gap-2 text-xs"><span className="subtle-panel px-3 py-2 text-center">صناعة يدوية</span><span className="subtle-panel px-3 py-2 text-center">تجهيز حسب الطلب</span><span className="subtle-panel px-3 py-2 text-center">مقاسات مختلفة</span><span className="subtle-panel px-3 py-2 text-center">تفاصيل قابلة للتخصيص</span></div>}<ProductDetailActions product={{ ...product, priceGuide: formatArabicPriceGuide(product.priceGuide) }} /></div>
