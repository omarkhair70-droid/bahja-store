...
export default async function ShopPage({ searchParams }: { searchParams: Promise<ShopSearchParams> }) {
  ...
  const filteredProducts = products.filter((p) => selectedCollection ? p.collectionSlug === selectedCollection : selectedCategory ? p.categorySlug === selectedCategory : true).sort((a, b) => (sortOrder[a.collectionSlug] ?? 99) - (sortOrder[b.collectionSlug] ?? 99));
  ...
  return <SectionShell title="المتجر" subtitle="تصاميم هادئة بلمسة يدوية. Bags first ثم الإكسسوارات والكانفس.">
    <div className="mb-5 flex gap-2 overflow-x-auto pb-2 whitespace-nowrap [&::-webkit-scrollbar]:hidden">{FILTERS.map((f)=><Link key={f.label} href={f.href} className={`rounded-full border px-3 py-1.5 text-xs sm:text-sm ${active?.label===f.label ? 'border-bahja-rose/40 bg-bahja-blush/45' : 'border-bahja-beige/70 bg-white/55'}`}><span>{f.label}</span><span className="ms-1 text-[10px] text-bahja-taupe">{f.sub}</span></Link>)}</div>
