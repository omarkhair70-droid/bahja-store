...
export default function ProductOptions({ isBag, size, setSize, quantity, setQuantity, note, setNote }: {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-bahja-beige/70 bg-white/75 p-4">
      {isBag && <div className="space-y-2"><p className="text-sm text-bahja-taupe">المقاس</p><div className="flex flex-wrap gap-2">{SIZES.map((s)=><button key={s.value} onClick={()=>setSize(s.value)} className={`rounded-full border px-4 py-1.5 text-sm ${size===s.value ? 'border-bahja-brown bg-bahja-brown text-white' : 'border-bahja-beige'}`}>{s.label}</button>)}</div></div>}
