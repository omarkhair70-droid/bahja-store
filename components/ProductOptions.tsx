...
export default function ProductOptions({ isBag, size, setSize, quantity, setQuantity, note, setNote }: {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-bahja-beige/60 bg-white/70 p-3.5 sm:p-4">
      ...
      <div className="space-y-2"><label className="text-sm text-bahja-taupe">اللون أو التفاصيل المطلوبة</label><textarea value={note} onChange={(e)=>setNote(e.target.value)} placeholder="اكتبي اللون أو أي تفاصيل تحبيها" className="min-h-20 w-full rounded-2xl border border-bahja-beige bg-bahja-ivory/70 p-3 text-sm outline-none focus:border-bahja-rose" /></div>
    </div>
  );
}
